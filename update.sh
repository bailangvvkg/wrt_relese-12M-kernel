...
my_seeting() {
    ...
    #开启内存回收补丁
    echo "CONFIG_KERNEL_SKB_RECYCLER=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_SKB_RECYCLER_MULTI_CPU=y" >> $BUILD_DIR/.config

    #编译器优化
    # if [[ $WRT_TARGET != *"X86"* ]]; then
    # 	echo "CONFIG_TARGET_OPTIONS=y" >> $BUILD_DIR/.config
    # 	echo "CONFIG_TARGET_OPTIMIZATION=\"-O3 -pipe -march=armv8-a+crypto+crc -mcpu=cortex-a53+crypto+crc -mtune=cortex-a53\"" >> $BUILD_DIR/.config
    # fi

    # eBPF
    echo "CONFIG_DEVEL=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_DEBUG_INFO=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_DEBUG_INFO_REDUCED=n" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_DEBUG_INFO_BTF=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_CGROUPS=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_CGROUP_BPF=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_BPF_EVENTS=y" >> $BUILD_DIR/.config
    echo "CONFIG_BPF_TOOLCHAIN_HOST=y" >> $BUILD_DIR/.config
    echo "CONFIG_KERNEL_XDP_SOCKETS=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_kmod-xdp-sockets-diag=y" >> $BUILD_DIR/.config
    echo "CONFIG_BPF_TOOLCHAIN_NONE=n" >> $BUILD_DIR/.config

    # BPFtool 支持 eBPF 程序 反汇编（disassembly）
    echo "CONFIG_PACKAGE_bpftool-full=y" >> $BUILD_DIR/.config
    ...
    # 想要剔除的
    # echo "CONFIG_PACKAGE_htop=n" >> $BUILD_DIR/.config
    # echo "CONFIG_PACKAGE_iperf3=n" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_luci-app-wolplus=n" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_luci-app-tailscale=n" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_luci-app-advancedplus=n" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_luci-theme-kucat=n" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_luci-app-mihomo=n" >> $BUILD_DIR/.config
    # 使用opkg替换apk安装器
    # echo "CONFIG_PACKAGE_opkg=y" >> $BUILD_DIR/.config
    # echo "CONFIG_OPKG_USE_CURL=y" >> $BUILD_DIR/.config
    # echo "# CONFIG_USE_APK is not set" >> $BUILD_DIR/.config
    # 可以让FinalShell查看文件列表并且ssh连上不会自动断开
    echo "CONFIG_PACKAGE_openssh-sftp-server=y" >> $BUILD_DIR/.config
    # 解析、查询、操作和格式化 JSON 数据
    echo "CONFIG_PACKAGE_jq=y" >> $BUILD_DIR/.config
    # base64 修改码云上的内容 需要用到
    echo "CONFIG_PACKAGE_coreutils-base64=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_coreutils=y" >> $BUILD_DIR/.config
    # 简单明了的系统资源占用查看工具
    echo "CONFIG_PACKAGE_btop=y" >> $BUILD_DIR/.config
    # 多网盘存储
    echo "CONFIG_PACKAGE_luci-app-alist=y" >> $BUILD_DIR/.config
    # 强大的工具Lucky大吉(需要添加源或git clone)
    echo "CONFIG_PACKAGE_luci-app-lucky=y" >> $BUILD_DIR/.config
    # 网络通信工具
    echo "CONFIG_PACKAGE_curl=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_tcping=y" >> $BUILD_DIR/.config
    # BBR 拥塞控制算法(终端侧)
    echo "CONFIG_PACKAGE_kmod-tcp-bbr=y" >> $BUILD_DIR/.config
    echo "CONFIG_DEFAULT_tcp_bbr=y" >> $BUILD_DIR/.config
    # 磁盘管理
    echo "CONFIG_PACKAGE_luci-app-diskman=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_cfdisk=y" >> $BUILD_DIR/.config
    # 其他调整
    # 大鹅
    echo "CONFIG_PACKAGE_luci-app-daed=y" >> $BUILD_DIR/.config
    # 大鹅-next
    # echo "CONFIG_PACKAGE_luci-app-daed-next=y" >> $BUILD_DIR/.config
    # 连上ssh不会断开并且显示文件管理
    echo "CONFIG_PACKAGE_opeh-sftp-server"=y
    # docker只能集成
    echo "CONFIG_PACKAGE_luci-app-dockerman=y" >> $BUILD_DIR/.config
    # qBittorrent
    # echo "CONFIG_PACKAGE_luci-app-qbittorrent=y" >> $BUILD_DIR/.config
    # 添加Homebox内网测速
    # echo "CONFIG_PACKAGE_luci-app-homebox=y" >> $BUILD_DIR/.config
    # V2rayA
    echo "CONFIG_PACKAGE_luci-app-v2raya=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_v2ray-core=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_v2ray-geoip=y" >> $BUILD_DIR/.config
    echo "CONFIG_PACKAGE_v2ray-geosite=y" >> $BUILD_DIR/.config
    # NSS的sqm
    # echo "CONFIG_PACKAGE_luci-app-sqm=y" >> $BUILD_DIR/.config
    # echo "CONFIG_PACKAGE_sqm-scripts-nss=y" >> $BUILD_DIR/.config
    # NSS MASH
    # echo "CONFIG_ATH11K_NSS_MESH=y" >> $BUILD_DIR/.config
    # 不知道什么 加上去
    # echo "CONFIG_PACKAGE_MAC80211_NSS_REDIRECT=y" >> $BUILD_DIR/.config
    # istore 编译报错
    # echo "CONFIG_PACKAGE_luci-app-istorex=y" >> $BUILD_DIR/.config
    # QuickStart
    # echo "CONFIG_PACKAGE_luci-app-quickstart=y" >> $BUILD_DIR/.config
    # filebrowser-go
    # echo "CONFIG_PACKAGE_luci-app-filebrowser-go=y" >> $BUILD_DIR/.config
    # 图形化web UI luci-app-uhttpd	
    echo "CONFIG_PACKAGE_luci-app-uhttpd=y" >> $BUILD_DIR/.config
    # 多播
    # echo "CONFIG_PACKAGE_luci-app-syncdial=y" >> $BUILD_DIR/.config
    # MosDNS
    echo "CONFIG_PACKAGE_luci-app-mosdns=y" >> $BUILD_DIR/.config
    # Natter2 报错
    # echo "CONFIG_PACKAGE_luci-app-natter2=y" >> $BUILD_DIR/.config
    # 文件管理器
    echo "CONFIG_PACKAGE_luci-app-filemanager=y" >> $BUILD_DIR/.config
    # 不要coremark 避免多线程编译报错
    # echo "CONFIG_PACKAGE_coremark=n" >> $BUILD_DIR/.config
    # 基于Golang的多协议转发工具
    echo "CONFIG_PACKAGE_luci-app-gost=y" >> $BUILD_DIR/.config
    # Go语言解析
    # echo "CONFIG_PACKAGE_golang=y" >> $BUILD_DIR/.config
    # Git
    echo "CONFIG_PACKAGE_git-http=y" >> $BUILD_DIR/.config
    # Nginx替换Uhttpd
    # echo "CONFIG_PACKAGE_nginx-mod-luci=y" >> $BUILD_DIR/.config
    # Nginx的图形化界面
    # echo "CONFIG_PACKAGE_luci-nginxer=y" >> $BUILD_DIR/.config
    # HAProxy 比Nginx更强大的反向代理服务器
    echo "CONFIG_PACKAGE_luci-app-haproxy-tcp=y" >> $BUILD_DIR/.config
    # Adguardhome去广告
    echo "CONFIG_PACKAGE_luci-app-adguardhome=y" >> $BUILD_DIR/.config
    # cloudflre速度筛选器
    echo "CONFIG_PACKAGE_luci-app-cloudflarespeedtest=y" >> $BUILD_DIR/.config
    # OpenClash
    echo "CONFIG_PACKAGE_luci-app-openclash=y" >> $BUILD_DIR/.config
    # nfs-kernel-server共享
    echo "CONFIG_PACKAGE_nfs-kernel-server=y" >> $BUILD_DIR/.config
    # Kiddin9 luci-app-nfs
    echo "CONFIG_PACKAGE_luci-app-nfs=y" >> $BUILD_DIR/.config
    # zoneinfo-asia tzdata（时区数据库）的一部分，只包含亚洲相关的时区数据 zoneinfo-all全部时区（体积较大，不推荐在嵌入设备）
    echo "CONFIG_PACKAGE_zoneinfo-all=y" >> $BUILD_DIR/.config
    # Caddy
    echo "CONFIG_PACKAGE_luci-app-caddy=y" >> $BUILD_DIR/.config
}
